 ...
 */
  ...
  register struct proc *low_rp = NIL_PROC;	/* lowest time user process */
  clock_t low_time = LONG_MAX;			/* lowest time */

  /* Check each of the scheduling queues except for the idle queue for ready
   * processes. The number of queues is defined in proc.h, and priorities are
   * set in the task table.
   */
  for (q = 0; q < NR_SCHED_QUEUES - 1; q++) {
	for (rp = rdy_head[q]; rp != NIL_PROC; rp = rp->p_nextready) {
		...
		/* User process. */
		if (rp->p_recent_time < low_time) {
			low_time = rp->p_recent_time;	/* record lowest */
			low_rp = rp;			/* time and proc */
		}
	}
  }
  /* The function has scanned through the scheduling queues and determined
   * that no system tasks wish to run while also recording the user process
   * (if any) with the lowest recent CPU time. Now test to see if there ARE
   * any user processes that wish to run.
   */
  if (low_rp != NIL_PROC) {
  	next_ptr = low_rp;			/* run process 'low_rp' next */
	bill_ptr = low_rp;			/* bill for system time */
	return;
  }
  /* No other process wants to run, so attempt to run the idle process. */
 ...
 */
  ...
  for (rp=BEG_PROC_ADDR; rp<END_PROC_ADDR; rp++) {
	  ...
	  rp->p_recent_time >>= 1;			/* decay recent cpu */
