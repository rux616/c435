 ...
 *				messaging_dmp                                *
 *===========================================================================*/
PUBLIC void messaging_dmp()
{
/* Messaging grid dump */

  register struct proc *rrp, *crp;
  static struct proc *oldrrp = BEG_PROC_ADDR, *oldcrp = BEG_PROC_ADDR;
  int r, row, col;
  ...
  printf("\nNumber of messages sent from process in given row to process in given column:\n");

  /* iterate through the proc table (rows of matrix) */
  for (rrp = oldrrp, row = 0; rrp < END_PROC_ADDR; rrp++) {
  	if (isemptyp(rrp)) continue;
	if (++row > 23) break;

	/* iterate through the message array (columns of matrix) */
  	for (crp = oldcrp, col = 0; crp < END_PROC_ADDR; crp++) {
		...
		if (row == 1) {
			if (col == 1)
				...
				if (strcmp(crp->p_name, "<unset>"))
					printf("%10s|", crp->p_name);
				else
					printf(" <pid> %3d|", crp->p_nr);
			}
		}
		else {
			if (col == 1) {
				if (strcmp(rrp->p_name, "<unset>"))
					printf("%13s|", rrp->p_name);
				else
					printf("    <pid> %3d|", rrp->p_nr);
			}
			else
				printf("%10u|", rrp->p_mess_sent[crp->p_nr +
					NR_TASKS]);
		}
	}
	printf("\n");
  }
  /* handle the paging logic */
#define LTR_PAGING 1
#if LTR_PAGING
  /* left-to-right, top-to-bottom paging */
  if (crp == END_PROC_ADDR) {
  	crp = BEG_PROC_ADDR;
	if (rrp == END_PROC_ADDR)	rrp = BEG_PROC_ADDR;
	else				printf("--more-- <Ù\r");
  }
  else {
	rrp = oldrrp;
	printf("--more-- >>\r");
...
#else
  /* top-to-bottom, left-to-right paging */
  if (rrp == END_PROC_ADDR) {
  	rrp = BEG_PROC_ADDR;
	if (crp == END_PROC_ADDR)	crp = BEG_PROC_ADDR;
	else				printf("--more-- ©>\r");
  }
  else {
	crp = oldcrp;
