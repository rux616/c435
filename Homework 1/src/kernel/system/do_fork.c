...
/* Handle sys_fork().  PR_ENDPT has forked.  The child is PR_SLOT. */
  ...
  *rpc = *rpp;				/* copy 'proc' struct */
  ...
  /* Because this is a copy of the parent process, message data is copied over
   * as well. This should be reset so we have a clean slate.
   */
  memset(rpc->p_mess_sent, 0, sizeof(rpc->p_mess_sent));

  /* Reset the number of messages sent by other processes to any previous
   * process that used the same pid.
   */
  for (rp = BEG_PROC_ADDR, i = rpc->p_nr + NR_TASKS; rp < END_PROC_ADDR; ++rp)
  	rp->p_mess_sent[i] = 0;
